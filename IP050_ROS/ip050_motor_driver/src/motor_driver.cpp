// motor_driver.cpp
#include <ip050_motor_driver/motor_driver.h>
  ...
  node_up_(false)
{
    ...
    torque_ = false;
    max_rpm_ = 0;
    rpm_pub_ = nh->advertise<ip050_msgs::RobotMotor>("/cur_rpm", 100);
...
void MotorDriver::set_node_mode_(const ControlWord mode){
    ...
    int left_motor_SDO = SDO + left_motor_id_;  // add SDO
    int right_motor_SDO = SDO + right_motor_id_;
    switch (mode)
    {
    ...
    case OP_DISENABLE:
        ...
        CAN_handler_->write_frame();
        break;
    case SET_VELOCITY_MODE:  // add 
        CAN_handler_->make_frame(left_motor_SDO, SET_VELOCITY_MODE_FRAME);
        CAN_handler_->write_frame();
        CAN_handler_->make_frame(right_motor_SDO, SET_VELOCITY_MODE_FRAME);
        CAN_handler_->write_frame();
        break;
    default:
        break;
...
void MotorDriver::cmd_rpm_callback(const ip050_msgs::RobotMotor::ConstPtr &msg){
...
bool MotorDriver::init(std::string aidlrobot)
{
    ...
    ros::Duration(0.2).sleep();
    set_node_mode_(SET_VELOCITY_MODE); // add velocity mode
    ros::Duration(0.2).sleep();
